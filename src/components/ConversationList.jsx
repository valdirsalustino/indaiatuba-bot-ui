...
const FilterIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"></polygon></svg>
);
...
export default function ConversationList({
  ...
  return (
    <div className="w-full md:w-1/3 lg:w-1/4 bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
      <header className="px-3 py-3 border-b border-gray-200 flex justify-between items-center">
        <div className="flex items-center min-w-0">
            <h1 className="text-lg font-bold text-gray-800 mr-2 truncate">Conversas</h1>
        ...
        <div className="flex items-center space-x-2 flex-shrink-0">
            {isDoctor && isCalendarEnabled && (
                <>
                    ...
                    <button onClick={onShowCalendar} title="Configurações do Calendário" className="text-gray-500 hover:text-blue-500 transition-colors">
                        <SettingsIcon />
                    </button>
                </>
            )}
            {(isAdmin || currentUser.role === 'Secretaria') && (
                <>
                    ...
                    {isAdmin && (
                        <>
                            <button onClick={onShowDashboard} title="Painel" className="text-gray-500 hover:text-blue-500 transition-colors">
                                <DashboardIcon />
                            </button>
                            <button onClick={onShowClientConfigs} title="Configurações do Cliente" className="text-gray-500 hover:text-blue-500 transition-colors">
                                <SettingsIcon />
                            </button>
                            <button onClick={onShowUserManagement} title="Gerenciar Usuários" className="text-gray-500 hover:text-blue-500 transition-colors">
                                <ManageUsersIcon />
                            </button>
                        </>
                    )}
            ...
            <button
              ...
              className={`transition-colors ${isFiltersOpen ? 'text-blue-500' : 'text-gray-500 hover:text-blue-500'}`}
            >
                <FilterIcon />
            </button>
            <button onClick={onLogout} title="Sair" className="text-gray-500 hover:text-red-500 transition-colors">
                <LogoutIcon />
            </button>
        </div>
      </header>
          ...
          return (
            ...
            >
              ...
              <div className="flex-grow overflow-hidden">
                ...
                {((showTopics && conv.topic) || (showLeads && conv.lead)) && (
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {showTopics && conv.topic && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-indigo-50 text-indigo-700 border border-indigo-150">
                        {conv.topic}
                      </span>
                    )}
                    {showLeads && conv.lead && (
